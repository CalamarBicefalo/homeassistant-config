...
class App(hass.Hass):
    async def helper_to_datetime(self, helper: Helper):
        """
    ...
    async def is_consuming_at_least(self, device: Entity, watts: int) -> bool:
        return await self.get_watt_consumption(device) >= watts

    async def get_watt_consumption(self, device: Entity) -> int:
        return int(float(await self.get_state(device)))

    async def is_on(self, device: Entity):
        state = await self.get_state(device)
        return state == states.ON or state == "playing"

    async def is_off(self, device: Entity):
        return await self.has_state(device, states.OFF)

    async def has_state(self, device: Entity | Helper, desired_state: str) -> bool:
        state = self.get_state(device)
        return (await state) == desired_state

    async def is_activity(self, helper: Helper, activity: Activity):
        return await self.has_state(helper, activity)

    async def get_activity_value(self, helper: Helper) -> Activity:
