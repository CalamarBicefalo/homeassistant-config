...
class MediaButtonsHandler:
    ...
    def on_forwards(self, callback: Callable[..., None]) -> None:
        self._app.listen_event(lambda *_: callback(), "zha_event", command='step', params=lambda x: x.get('step_mode') == 0,
                               device_ieee=self.device_ieee)

    def on_backwards(self, callback: Callable[..., None]) -> None:
        self._app.listen_event(lambda *_: callback(), "zha_event", command='step', params=lambda x: x.get('step_mode') == 1,
                               device_ieee=self.device_ieee)

    def on_volume_up(self, callback: Callable[..., None]) -> None:
        self._app.listen_event(lambda *_: callback(), "zha_event", command=lambda x: x in ['move', 'move_with_on_off'],
                               params=lambda x: x.get('move_mode') == 0,
                               device_ieee=self.device_ieee)

    def on_volume_down(self, callback: Callable[..., None]) -> None:
        self._app.listen_event(lambda *_: callback(), "zha_event", command=lambda x: x in ['move', 'move_with_on_off'],
                               params=lambda x: x.get('move_mode') == 1,
                               device_ieee=self.device_ieee)
